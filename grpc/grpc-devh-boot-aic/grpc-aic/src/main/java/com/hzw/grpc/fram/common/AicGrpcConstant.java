package com.hzw.grpc.fram.common;

/**
 * @ClassName AicGrpcConstant
 * @Description TODO
 * @Author houzw
 * @Date 2023/7/21
 **/
public class AicGrpcConstant {
    public static final String AIC_GRPC_PROXY_BEAN_PREFIX = "aicgrpc_proxy_";

    public static final String AIC_GRPC_STUB_TYPE_ASYNC    = "Async";
    public static final String AIC_GRPC_STUB_TYPE_BLOCKING = "Blocking";
    public static final String AIC_GRPC_STUB_TYPE_FUTURE   = "Future";
}
